...
// Klystron is a server-side proxy: it serves rewritten remote pages from Bardo's
// own origin, so it must run ahead of the global security headers below (a strict
// CSP + no-referrer would break proxied content). It sets its own headers.
